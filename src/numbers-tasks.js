...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scoliarProduct = x1 * x2 + y1 * y2;
  const modulFirst = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modulSecond = Math.sqrt(x2 ** 2 + y2 ** 2);
  const res = scoliarProduct / (modulFirst * modulSecond);
...
function isPowerOfTwo(num) {
  const a = Math.log(num) / Math.log(2);
...
function getRandomInteger(min, max) {
  const choise = min + Math.random() * (max + 1 - min);
  return Math.floor(choise);
}
